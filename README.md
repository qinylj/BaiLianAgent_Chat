...
```
agent-chat/
├─ server.mjs               # 零依赖服务端：静态托管 + 5 网关接口代理 + 大模型 OpenAI 代理 + 流式归一化 + 历史落盘
├─ package.json
├─ config/
...
- **缺 key 时不会静默发一个空凭证**，而是明确报错并点名是谁缺：
  ...
  ```
- **5 个接口全都按「会话所属智能体」选 key**。`clearSession` / `deleteSession` / `feedback` 只有 `sessionId`，浏览器会额外带上 `agentId`；服务端另有一份 `sessionId → agentId` 的内存记忆做兜底（老版本前端 / 第三方调用）。这些都推不出来时退回全局配置 —— **不会随便挑一个智能体**，那会把别人的专属 key 用到错误的会话上。
